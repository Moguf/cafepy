...
class ReadDCD(FileIO):
    ...
    def __getitem__(self,num):
        self._file.seek(0)
        self.readHeader()
        stepsize =(4 + 4 * self._header.nmp_real) * 3
        """
        File contents.        
        bytesize(long integer) x cordinate(double) * The Number of atoms
        bytesize(long integer) y cordinate(double) * The Number of atoms
        bytesize(long integer) z cordinate(double) * The Number of atoms
        
        Example:(test.dcd)
        228(long int) 57 * 4(x cordinate double) 
        228(long int) 57 * 4(y cordinate double)
        228(long int) 57 * 4(z cordinate double)
        ===> stepszie = (228 + 4) * 3 
